...
@mcp.tool()
async def list_sample_files() -> str:
    """List only the compact bundled sample IDF and weather files used for first-run workflows."""
    try:
        files = ep_manager.list_available_files(False, False)
        return f"Bundled sample files:\n{files}"
    except Exception as e:
        logger.error(f"Error listing bundled sample files: {str(e)}")
        return f"Error listing bundled sample files: {str(e)}"
...
@mcp.tool()
async def calculate_energy_performance(output_directory: str) -> str:
    """Calculate evidence-backed EPI/EUI, energy, area, and diagnostics from a completed simulation.

    Args:
        output_directory: Directory returned by run_energyplus_simulation.

    Returns:
        JSON with site/source energy, total and conditioned area, kWh/m2/year EUI, warnings, and errors.
    """
    try:
        result = ep_manager.calculate_energy_performance(output_directory)
        return f"Energy performance analysis completed:\n{json.dumps(result, indent=2)}"
    except Exception as e:
        logger.error(f"Error calculating energy performance: {str(e)}")
        return f"Error calculating energy performance: {str(e)}"
