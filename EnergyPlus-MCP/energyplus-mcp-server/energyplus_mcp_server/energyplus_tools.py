...
class EnergyPlusManager:
    ...
    def calculate_energy_performance(self, output_directory: str) -> Dict[str, Any]:
        """Extract site/source energy, floor area, EUI/EPI, and error counts from EnergyPlus outputs."""
        ...
        conditioned_area = metrics["conditioned_building_area_m2"]
        metrics["site_eui_kwh_m2_year_total_area"] = round(site_gj * 277.7777778 / total_area, 3) if site_gj is not None and total_area else None
        metrics["site_eui_kwh_m2_year_conditioned_area"] = round(site_gj * 277.7777778 / conditioned_area, 3) if site_gj is not None and conditioned_area else None

        diagnostics = {"warnings": None, "severe_errors": None, "fatal_errors": None, "completed_successfully": None}
        error_files = sorted(output_dir.glob("*.err"))
        if error_files:
            error_text = error_files[0].read_text(encoding="utf-8", errors="replace")
            match = re.search(r"Completed Successfully--\s*(\d+) Warning;\s*(\d+) Severe Errors", error_text)
            diagnostics["completed_successfully"] = "EnergyPlus Completed Successfully" in error_text
            diagnostics["fatal_errors"] = len(re.findall(r"\*\*\s*Fatal\s*\*\*", error_text, re.I))
            if match:
